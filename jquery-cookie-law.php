<?php
/*
Plugin Name: ChatMe Cookie Consent
...
class jcookielawJS {
    ...
    private $default     = array(
				...
				'plugin_options_key'    => 'jquery-cookie-law',
                		...
                		'policyURL'             => 'http://cookiesandyou.com/',
				'expireDays'            => 90,
				'theme'			=> 'classic',
				'target'		=> '_self',
				'position'		=> 'bottom',
	...
	function __construct() {
		add_action( 'plugins_loaded', array( $this, 'my_jcookielaw_init' ) );
		}

	function my_jcookielaw_init() {
      		load_plugin_textdomain( 'jquery-cookie-law');
		add_action( 'wp_enqueue_scripts', 		array( $this, 'get_jcookielaw_head') );
		add_action( 'admin_menu',    			array( $this, 'jcookielaw_admin_menu') );
		add_action( 'admin_init',    			array( $this, 'jcookielaw_admin_init') );
	...
	function get_jcookielaw_head() {
		...
		$actual = apply_filters( 'jcookielaw_actual', wp_parse_args( $setting, $this->default ) );
    ...
    function jquery_cookie_law_add_help_tab () {
        ...
        $screen->set_help_sidebar(
               			'<p><strong>' . esc_html__('Other Resources', 'jquery-cookie-law') . '</strong></p><p><a href="https://silktide.com/tools/cookie-consent/" target="_blank">' . esc_html__('Cookie Consent Official Site', 'jquery-cookie-law') . '</a></p><p><a href="http://chatme.im" target="_blank">' . esc_html__('ChatMe Site', 'jquery-cookie-law') . '</a></p>'
                             );
      	      	}

    function jcookielaw_admin_menu() {
        $my_admin_page = add_options_page( esc_html__('ChatMe Cookie Consent Options', 'jquery-cookie-law'), esc_html__('ChatMe Cookie Consent', 'jquery-cookie-law'), 'manage_options', $this->default['plugin_options_key'], array($this, 'jcookielaw_options') );
		add_action('load-'.$my_admin_page, array( $this, 'jquery_cookie_law_add_help_tab') );
    }

    function jcookielaw_admin_init() {
        ...
        <form method="post" action="options.php">
            <?php settings_fields( 'jquery-cookie-law' ); ?>
                <table class="form-table">
    	            <tr valign="top">
                        <th scope="row"><label for="message"><?php esc_html_e('Message', 'jquery-cookie-law'); ?></label></th>
                            <td>
				                <textarea aria-describedby="message-description" name="message" id="message" class="large-text code" rows="3"><?php echo get_option('message'); ?></textarea>
                    ...
                    <tr valign="top">
                        <th scope="row"><label for="pbackground"><?php esc_html_e('Background Color', 'jquery-cookie-law'); ?></label></th>
                            <td>
                                <input class="pbackground" aria-describedby="pbackground-description" type="text" id="pbackground" name="pbackground" value="<?php echo get_option('pbackground'); ?>" data-default-color="<?php echo $this->default['pbackground']; ?>" />
                            </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row"><label for="ptext"><?php esc_html_e('Text Color', 'jquery-cookie-law'); ?></label></th>
                            <td>
                                <input class="ptext" aria-describedby="ptext-description" type="text" id="ptext" name="ptext" value="<?php echo get_option('ptext'); ?>" data-default-color="<?php echo $this->default['ptext']; ?>" />
                            </td>
                    </tr>
                    <tr valign="top">
                    ...
                    <tr valign="top">
                        <th scope="row"><label for="bbackground"><?php esc_html_e('Button Color', 'jquery-cookie-law'); ?></label></th>
                            <td>
                                <input class="bbackground" aria-describedby="bbackground-description" type="text" id="bbackground" name="bbackground" value="<?php echo get_option('bbackground'); ?>" data-default-color="<?php echo $this->default['bbackground']; ?>" />
                            </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row"><label for="btext"><?php esc_html_e('Button Text Color', 'jquery-cookie-law'); ?></label></th>
                            <td>
                                <input class="btext" aria-describedby="btext-description" type="text" id="btext" name="btext" value="<?php echo get_option('btext'); ?>" data-default-color="<?php echo $this->default['btext']; ?>" />
                            </td>
                    </tr>
                    <tr valign="top">
				...
				<select name="policyURL" id="policyURL" aria-describedby="policyURL-description"> 
 					...
 					<option value="http://cookiesandyou.com/" <?php selected('policyURL', get_option('http://cookiesandyou.com/')); ?>>cookiesandyou.com</option> 
 				<?php 
  					$pages = get_pages(); 
				...
				?>
					...
					<option value="_self" <?php selected('target', get_option('_self')); ?>><?php esc_html_e('Same Window', 'jquery-cookie-law'); ?></option>
					<option value="_blank" <?php selected('target', get_option('_blank')); ?>><?php esc_html_e('New Window', 'jquery-cookie-law'); ?></option>
				</select>
                <p class="description" id="policyURL-description"><?php esc_html_e('Select the page of Cookies/Privacy Policy.', 'jquery-cookie-law'); ?> - <?php esc_html_e('Not have a Privacy Policy? Generate one with', 'jquery-cookie-law'); ?> <a href="http://iubenda.refr.cc/7J8242P" target="_blank">iubenda.com</a> <?php esc_html_e('with 10% discount.', 'jquery-cookie-law'); ?></p>
                            </td>
                    </tr>
                    <tr valign="top">
        	...
        	<tr valign="top">
        		...
        		<td>
					<select id="theme" name="theme">
						<option value="classic" <?php selected('theme', get_option('classic')); ?>><?php esc_html_e('Classic', 'jquery-cookie-law'); ?></option>
						<option value="edgeless" <?php selected('theme', get_option('edgeless')); ?>><?php esc_html_e('Edgeless', 'jquery-cookie-law'); ?></option>
					</select>
				</td>
        	</tr>
        	...
        	<tr valign="top">
        		...
        		<td>
					<select id="position" name="position">
						<option value="bottom" <?php selected('position', get_option('bottom')); ?>><?php esc_html_e('Bottom', 'jquery-cookie-law'); ?></option>
						<option value="top" <?php selected('position', get_option('top')); ?>><?php esc_html_e('Top', 'jquery-cookie-law'); ?></option>
						<option value="bottom-left" <?php selected('position', get_option('bottom-left')); ?>><?php esc_html_e('Floating left', 'jquery-cookie-law'); ?></option>
						<option value="bottom-right" <?php selected('position', get_option('bottom-right')); ?>><?php esc_html_e('Floating right', 'jquery-cookie-law'); ?></option>
					</select>
				</td>
        	</tr> 
